...
const WeaponUpdate = () => {
  ...
  const fetch = () => {
    axios
      ...
      .then((response) => {
        ...
        response.data.operator.map((item) => {
          setOperator((prevArray) => [...prevArray, item._id]);
        });
    ...
    axios.get(`http://localhost:4000/operator/all`).then((response) => {
      response.data.map((item) => {
        ...
        setOperators((pervArray) => [...pervArray, newItem]);
      });
    });
  };
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .put(`http://localhost:4000/weapon/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: formData,
      })
      .then(() => navigate(`/weapon/${id}`))
      .catch((e) => console.error("Error: ", e));
  };
          ...
          <FormControl>
            ...
            <FormControl>
              ...
              <RadioGroup
                ...
                name="primary"
                defaultValue={weapon.primary ? "primary" : "secondary"}>
                <FormControlLabel
                  value="primary"
                  control={<Radio />}
                  label="주무기"
            ...
            <FormControl>
              ...
              <OutlinedInput
                ...
                required
                inputProps={{ min: 0 }}
                defaultValue={weapon.capacity}
              />
              <FormHelperText id="capacity">
                약실을 빼고 입력하세요
              </FormHelperText>
            </FormControl>
            <FormControl>
